...
grafico.iris <- ggplot(data=iris.tidy, aes(x=Species, y=Value)) + geom_point()

grafico.iris2 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, color=Part)) + geom_point() + facet_grid(.~Measure )

#grafico.iris2 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, color=Part)) + geom_point() + facet_grid(Measure~. )
#Creo que se visualiza mejor si los mostramos uno a la izquierda y uno a la derecha

grafico.iris2 <- ggplot(data=iris.tidy, aes(x=Species, y=Value, color=Part)) + geom_jitter() + facet_grid(.~Measure )
